        ...
        {domain && domain !== "portal" ? (
            ...
            {domain === "desk" ? (
              <div className="mt-5">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-3">
                  {config.deskSurfaces.map((surface) => <div key={surface} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm text-white/75">{surface}</div>)}
                </div>
              </div>
            ) : (
          ...
          <section className="mt-5 grid gap-3 sm:grid-cols-2">
            {ZCOS_SHARED_DOMAINS.map((item) => (
              <button key={item} type="button" className="flex min-h-[68px] items-center justify-between rounded-2xl border border-white/10 bg-white/[0.035] px-5 text-left transition hover:bg-white/[0.07]" onClick={() => navigate(item === "portal" ? "/" : `${root}/${item}`)}>
                <span><span className="block text-sm font-medium">{item === "desk" ? `${config.desk} Desk` : domainLabel(item)}</span>{item === "desk" && <span className="mt-1 block text-xs text-white/40">{config.deskSurfaces.join(" · ")}</span>}</span>
