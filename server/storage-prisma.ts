...
export class PrismaStorage {
  ...
  async listMemory(ownerUserId: string, galaxyId: string) {
    return prisma.memoryRecord.findMany({ where: { ownerUserId, galaxyId, lifecycleState: { notIn: ['rejected', 'superseded', 'forgotten'] } }, orderBy: { updatedAt: 'desc' } });
  }
  async createMemory(data: { ownerUserId: string; galaxyId: string; memoryType: string; canonicalName: string; content: string; lifecycleState?: string }) {
    return prisma.memoryRecord.create({ data: { ...data, lifecycleState: data.lifecycleState || 'confirmed' } });
  }

  async listKnowledge(ownerUserId: string, galaxyId: string) {
    return prisma.knowledgeRecord.findMany({ where: { ownerUserId, galaxyId, lifecycleState: { notIn: ['rejected', 'superseded', 'deprecated'] } }, orderBy: { updatedAt: 'desc' } });
  }
  async createKnowledge(data: { ownerUserId: string; galaxyId: string; objectType: string; canonicalName: string; summary?: string; originClass: string; lifecycleState?: string }) {
    return prisma.knowledgeRecord.create({ data: { ...data, lifecycleState: data.lifecycleState || 'candidate' } });
  }

  async hasPartitionGrant(input: { ownerUserId: string; sourceGalaxyId: string; targetGalaxyId: string; authorityKind: 'memory' | 'knowledge'; permission: 'read' | 'write' }) {
    const now = new Date();
